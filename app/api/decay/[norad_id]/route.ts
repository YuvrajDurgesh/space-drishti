...
export async function GET(
  ...
  const { norad_id } = await context.params;
  const noradId = parseInt(norad_id, 10);
  if (isNaN(noradId) || noradId <= 0) {
    return NextResponse.json(
      {
        error: "norad_id must be a positive integer",
  ...
  const maxDays = Math.min(
    Math.max(
      1,
      parseInt(req.nextUrl.searchParams.get("max_days") ?? "3650", 10),
    ),
    3650,
  );
  ...
  if (result.predicted_decay_at) {
    ...
    await supabase.from("decay_predictions").upsert(
      {
        norad_id: noradId,
